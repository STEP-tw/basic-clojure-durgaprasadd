...
(defn reduce'
  ...
  ([f init coll] (loop [coll coll res init]
                   (if (empty? coll) res (recur (rest coll) (f res (first coll)))))))
...
(defn count'
  ...
  {:level        :easy
   ...
   :dont-use     '[count]
   :implemented? true}
  [coll] (loop [coll coll res 0]
           (if (empty? coll) res (recur (rest coll) (inc res)))))
...
(defn dedupe'
  ...
  [coll]
  (if-let [coll (seq coll)]
    (let [current (first coll) rest-coll (rest coll)]
      (lazy-seq (if-not (= current (first rest-coll))
                  (cons current (dedupe' rest-coll))
                  (dedupe' rest-coll)))) []))
...
(defn union
  ...
  {:level        :easy
   :use          '[remove into set ->>]
   :implemented? true}
  [coll1 coll2] (concat coll1 (difference coll1 coll2)))
...
(def
  ...
  Note this is a def, not a defn"
  (for [x (range -1 2) y (range -1 2) :when (or (not= x 0) (not= x y))] [x y]))
...
(defn sqr-of-the-first
  ...
  {:level        :easy
   ...
   :implemented? true}
  [coll] (let [first-num (first coll)] (map (constantly (* first-num first-num)) coll)))
